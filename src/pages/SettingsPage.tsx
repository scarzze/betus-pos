  ...
  return (
    <div className="p-6">
          ...
          <div key={section.title} className="glass-card">
            ...
            <div className="grid gap-4 p-5 sm:grid-cols-2">
              {section.fields.map((field) => (
                <div key={field.label}>
                  <label className="mb-1.5 block text-sm font-medium text-muted-foreground">{field.label}</label>
                  <input
                    type={field.type}
      ...
      <div className="mt-6 flex justify-end">
        <button className="rounded-lg gradient-orange px-6 py-2.5 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90">
          Save Settings
        </button>
      </div>
    </div>
